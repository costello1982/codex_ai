...
You can containerize the application using [Podman](https://podman.io/) with the provided helper scripts.

1. Ensure Podman is installed and that your user can run rootless containers.
2. From the project root, execute the script that matches your platform:
...
3. Visit http://127.0.0.1:8000 to access the UI. Use `Ctrl+C` (Bash) or `Ctrl+Break` (PowerShell) in the terminal to stop the container.

### Customizing the Podman run

The scripts honor a few optional environment variables:
